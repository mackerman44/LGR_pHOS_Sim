...
dabom_hnc_phos = tab_nums('dabom_hnc_phos',
                          "Estimates of adundance for ad-intact steelhead including wild and hatchery (HNC), spawn years 2017-2019 and estimates of the proportion of hatchery origin spawners (pHOS) with standard error (SE) and coefficient of variation (CV) that takes into account uncertainty in the abundance estimates.")
...
From this plot, it is clear that when *pHOS* is high, then regardless of the CV of wild or hatchery escapement, reasonable precision of *pHOS* will be obtained. However, when *pHOS* is low, the precision of wild and hatchery escapement must be very good to obtain reasonable precision for *pHOS*. At very low levels of *pHOS*, very precise estimates of hatchery and wild escapement are necessary.
...
# model for estimating CV of estimate, based on number of tags detected
n_tags_mod = lm(log(cv) ~ log(n_tags),
                data = wild_est)

n_tags_mod = glm(cv ~ log(n_tags),
                 family = gaussian(link = 'log'),
                 data = wild_est)

# n_tags_mod = glm(cv ~ n_tags,
#                  family = gaussian(link = 'log'),
#                  data = wild_est)
