...
We predicted the CV of *pHOS* across a range of values for both hatchery escapement CV (0 - 1.5) and *pHOS*. The results are shown in `r fig_nums('se_phos_fig', display = 'c')`.
...
The general patterns show that as *pHOS* increases, the precision of *pHOS* improves for a given precision level of wild and hatchery escapement estimates. Also, as the precision of the wild estimates improves (i.e. smaller CV, to the left on `r fig_nums('se_phos_fig', display = 'c')`), there are more scenarios under which the precision of the *pHOS* estimate is reasonable. As a reference, the CVs of the estimates for spring/summer Chinook escapement to the Lemhi range from 0.1 to 0.25 for years 2010 - 2018. When *pHOS* is quit low or quite high (towards the bottom or top of `r fig_nums('se_phos_fig', display = 'c')`), then regardless of the CV of wild or hatchery escapement, reasonable precision of *pHOS* will be obtained. However, as *pHOS* decreases and we move towards streams with mostly wild fish, it will require very precise estimates of hatchery and wild escapement to obtain a resonable estimate of *pHOS*.
...
sim_df %>%
  mutate(seFct = paste0('SE < ', seThres, '%')) %>%
  mutate_at(vars(seFct),
            list(as.factor)) %>%
  filter(SE > 0) %>%
  ggplot(aes(x = cvW,
             y = pHOS)) +
...
We modeled the relationship between the number of wild tags detected within a population and the CV of the wild abundance estimate using a log-log model (`r fig_nums("tags_cv_fig", display = 'c')`). We assumed the same relationship holds for hatchery tags detected. 
...
lgr_tags_df = hatch_cv %>%
  ...
  select(MPG_DPS:pHOS, conv_rate, seThres, maxHcv, n_tags, LGR_tags) %>%
