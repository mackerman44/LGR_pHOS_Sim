...
```{r stray-rates-v2, eval = F, out.width = "800px", fig.align = "center", fig.cap = fig_nums('stray_rates')}
knitr::include_graphics("../images/stray_rate_table.png")
```
...
The results from our mathematical exploration are shown in `r fig_nums('se_phos_fig', display = 'c')`. We evaluated the maximum CV for a hatchery abundance estimate that would provide a given level of precision (measured as SE) across varying levels of CV of wild escapement and *pHOS*. Further, we evaluated four different CV thresholds: 2%, 5%, 10%, and 25%. The same general patterns hold for all four standard error thresholds evaluated:

* It is easiest to meet SE threshholds when *pHOS* is very small or very large in that a wide range of precision in wild and hatchery estimates will work.
...
In other words, as the precision of the wild estimates improve (i.e., smaller CV, to the left of each facet in `r fig_nums('se_phos_fig', display = 'c')`), there are more scenarios under which the precision of the *pHOS* estimate is reasonable. For instance, the CVs of the estimates for spring/summer Chinook escapement to the Lemhi River range from 0.10 to 0.25 for spawn years 2010 - 2018. When *pHOS* is quite low or quite high (i.e., towards the bottom or top of each facet in `r fig_nums('se_phos_fig', display = 'c')`), then regardless of the CV of wild or hatchery escapement, a reasonable precision of *pHOS* can be obtained. For reference, estimates of *pHOS* and the CVs of the corresponding wild escapement estimates from `r tab_nums('dabom_hnc_phos', display = 'c')` are shown as open red circles in `r fig_nums('se_phos_fig', display = 'c')`.
...
We then examined the proportion of TRT populations that would have a reliable estimate of *pHOS* under different tagging numbers at Lower Granite Dam. Assuming a certain number of tags were deployed at Lower Granite, we determined which populations would have a standard error of *pHOS* below variousl thresholds, according to `r tab_nums("lgr_tags_tab", display = 'c')`. We then transformed that into a percent of all TRT populations where we expect to estimate *pHOS* (EXCLUDING those with no observed strays: `r paste(no_strays, collapse = ', ')`). This included a total of `r filter(lgr_tags_df, conv_rate > 0) %>% summarise(tot_trt = n_distinct(TRT)) %>% pull(tot_trt)` TRT steelhead populations. Those results are shown in `r fig_nums("trt_prop_fig", display = 'c')`. In other words, the y-axis of `r fig_nums("trt_prop_fig", display = 'c')` only pertains to monitored TRT populations where we have previously observed an HNC stray. Of the `r filter(lgr_tags_df, conv_rate > 0) %>% summarise(tot_trt = n_distinct(TRT)) %>% pull(tot_trt)` populations evaluated, those not shown on a given facet are off the right side of the plot (e.g., more tags need to be deployed at LGR to estimate a precise *pHOS*).
