...
### Section 3: Simulations based on Section 2

We will focus on **some subset of important populations defined by IDFG**. From the DABOM estimates of wild escapement over the last ten years, we will calculate the average CV of the wild estimate for each population, so we know where we fall on the x-axis in Figure \@ref(fig:cv-phos-fig). We will then use the average estimate of pHOS from Table \@ref(tab:dabom-hnc-phos) for each population, to determine where on the y-axis we are for each population. We will also assume a 5% pHOS and 10% pHOS, based on recovery goals (***Is this necessary?***). This will tell us what CV is required for an estimate of hatchery fish for each population, and we will translate that into a number of tags we'd like to detect in each population.

Brian Leth has provided a table showing the percent of hatchery adults, tagged as juveniles, that are detected within each population out of all the hatchery tags that are detected at Lower Granite. We will divide the number of detections we require for each population by these percentages to determine how many tags we would expect to need to put out at Lower Granite in order to acheive those precision goals. 
...
<!---
These are initial HNC abundance estimates from DABOM for spawn years 2017. I've added for 1/9/2020 meeting with IDFG. Next, we need to roll these up to TRT populations at the same scales that we estimate W abundance in the SnakeBasinFishStatus Git repo. Then, Kevin will determine the appropriate way to merge W and HNC abundance estimates from DABOM to estimate pHOS with SE and CV. Then summarize and report results. That should be it for this section.
-->
