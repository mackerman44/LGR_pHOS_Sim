...
There are a number of TRT populations for both species that currently lack any PIT tag detection infrastructure, and are therefore not represented in DABOM estimates at all. These are listed in `r tab_nums('no_monitor_tab', display = 'c')`.
...
`r fig_nums('good_cv_det_fig', display = 'c')` shows how such increases would impact the number of tags needed to be deployed. The main message from this analysis is that improving detection efficiency, even by as much as 20% (which is actually impossible in most populations, as that would increase it to above 100%), does not reduce the number of tags needed very much. This is partly due to the fact that most populations already have high detection efficiency. However, there may be certain populations where improving the detection infrastructure could lead to gains in the precision of the abundance estimates (`r fig_nums('det_prob_fig', display = 'c')`).
...
These results suggest we need to detect a minimum of about 50 tags per TRT population to achieve acceptable precision in our abundance estimates at the TRT spatial scale. Additionally, `r fig_nums('perc_good_cv', display = 'c')` and `r fig_nums('perc_good_cv_zoom', display = 'c')` can be interpreted as follows: Assuming a given TRT population contains IPTDS infrastructure intended for natural origin abundance monitoring and 4,000 PIT tags are deployed at LGR, we'd expect that about 75% (Chinook) and 60% (steelhead) of the TRT populations monitored with PIT tags would have abundance estimates with "good" precision (i.e. CV $\leq$ 0.15).
