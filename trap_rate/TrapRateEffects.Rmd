...
assign_POP_GSI <- function(species, configuration, site_df){
  ...
  load('../../SnakeBasinFishStatus/data/ConfigurationFiles/Snake_POP_metadata.rda')
...
prop_tags_lgr_fig = fig_nums('prop_tags_lgr_fig',
                             'The proportion of all tags deployed at LGR that were later detected within any TRT population each year, by species.')
...
perc_good_cv_v2 = fig_nums('perc_good_cv_v2',
...
# # what popualation and MGP are each site associated with?
# # load configuration file
# load('../../SnakeBasinFishStatus/data/ConfigurationFiles/site_config.rda')
# 
...
We want to note that reasonably precise estimates of abundance, by sex and total age, are also a desired outcome as they are necessary to construct brood tables and estimate population productivity. However, note that it will not require 50 female tags detected within a population to achieve reasonable precision for female abundance. Rather, we will take the total abundance estimate to a TRT population and multiply it by a binomial (female vs. male) proportion to generate abundance by females and males. The binomial proportions will be estimated from all of the tags detected in that TRT population. If we have about 50 total tags detected in a populaion for a good abundance estimate, the proportion female or male will be fairly precise because 50 is a good sample size for proportions, especially when the proportion is somewhere near 50%. Therefore, the CV for the number of females in that population will be slightly larger than the CV of the total abundance, but probably not by much. 
...
Next, `r tab_nums('lgr_tags_needed', display = 'c')` summarizes the total number of tags that would need to be deployed at LGR to achieve a precise abundance estimate for any given population. `r tab_nums('lgr_tags_needed', display = 'c')` only includes TRT populations which contain IPTDS intended for natural origin population abundance monitoring. 
...
One way to increase the number of tags detected in a given population is to increase the total number of tags deployed at Lower Granite, but another could be to increase the probability of detecting a tag assuming it does arrive in that population. This could be done by upgrading older in-stream PIT tag detection equipment, re-deploying it more effectively, or adding detection sites within the population. Without exploring specific ways that detection efficiency could be increased within each population (or even whether it could be increased by the amounts suggested) we evaluated the potential effect on the number of total tags needed to be deployed at Lower Granite if the detection probability was increased by 5, 10 or 20% across the basin. 

`r fig_nums('perc_good_cv_v2', display = 'c')` shows how such increases would impact the number of tags needed to be deployed. The main message from this analysis is that improving detection efficiency, even by as much as 20% (which is actually impossible in most populations, that would increase it to above 100%), does not reduce the number of tags needed very much. This is partly due to the fact that most populations already have high detection efficiency. However, there may be some populations where improving the detection infrastructure could lead to gains in the precision of the abundance estimates (`r fig_nums('det_prob_fig', display = 'c')`).

```{r perc_good_CV_v2, fig.cap = perc_good_cv_v2}
...
These results suggest we need to detect a minimum of about 50 tags per TRT population to achieve acceptable precision in our abundance estimates at the TRT spatial scale. Additionally, `r fig_nums('perc_good_cv', display = 'c')` could be interpreted as follows: Assuming a given TRT population contains IPTDS infrastructure intended for natural origin abundance monitoring and 4,000 PIT tags deployed at LGR, we'd expect that about 60-75% of all TRT population abundance estimates would have 'good' precision (i.e. CV $\leq$ 0.15).
