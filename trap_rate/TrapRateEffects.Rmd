...
remove_t = read_excel('../data/TRT_Pop_Considerations.xlsx') #%>%
  # # remove steelhead in CRLOL for 2019
  # bind_rows(tibble(Species = 'Steelhead',
  #                  TRT = "CRLOL-s",
  #                  SiteIDs = c('LC1, LC2'),
  #                  `2019` = 'Y',
  #                  Notes = c('LC1 and LC2 lost during the 2018-2019 season.')))
...
# modeling number of tags detected
mod_df = pop_est %>%
  group_by(Species) %>%
  nest() %>%
...
# modeling proportion of tags detected
prop_mod_df = pop_est %>%
  mutate_at(vars(spawn_yr),
            list(~ as.integer(as.character(.)))) %>%
  left_join(tags_deploy) %>%
...
#               fullrange = T) +
#   facet_wrap(~ Species) +
#   scale_y_continuous(limits = c(-0.01, 1.5)) +
...
pop_est %>%
  ...
  geom_hline(yintercept = 0.15,
             linetype = 2) +
  geom_line(data = prop_mod_df %>%
              gather(model, mod_fit, starts_with('mod')) %>%
              mutate(pred = map(.x = mod_fit,
                                .f = function(x) {
                                  tibble(prop_tags = seq(0, 0.1, by = 0.001)) %>%
                                    mutate(pred_cv = predict(x, 
                                                             newdata = .,
                                                             type = 'response'))
                                })) %>%
